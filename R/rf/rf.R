# packages
# install.packages(c("lifecycle", "rsample", "spatialsample", "dplyr", "pROC", "rgdal", "sf", "ggplot2", "pscl", "xts", "ISOweek", "glmnet", "raster", "PerformanceAnalytics"))
library(ranger)
library(rsample)
...
form <- MIRAll ~ chirpsMean + Humidity.Max + Humidity.Min + Specific.Humidity + Temp.Mean.K + VPD.kPa + Precip.Mean.mm...day + EVI + NDVI + jrcStandingWater + irrWater + eddi14d + eddi180d + eddi1y + eddi270d + eddi2y + eddi30d + eddi5y + eddi90d + pdsi + spei14d + spei180d + spei1y + spei270d + spei2y + spei30d + spei5y + spei90d + spi14d + spi180d + spi1y + spi270d + spi2y + spi30d + spi5y + spi90d + z

mirpir <- mirpir_train[c(c("MIRAll", "fold", "chirpsMean" , "Humidity.Max" , "Humidity.Min" , "Specific.Humidity" , "Temp.Mean.K" , "VPD.kPa" , "Precip.Mean.mm...day" ,"EVI", "NDVI" , "jrcStandingWater" , "irrWater" , "eddi14d" , "eddi180d" , "eddi1y" , "eddi270d" , "eddi2y" , "eddi30d" , "eddi5y" , "eddi90d" , "pdsi" , "spei14d" , "spei180d" , "spei1y" , "spei270d" , "spei2y" , "spei30d" , "spei5y" , "spei90d" , "spi14d" , "spi180d" , "spi1y" , "spi270d" , "spi2y" , "spi30d" , "spi5y" , "spi90d" , "z"))]

mirpir_final <- mirpir_test[c(c("MIRAll", "fold", "chirpsMean" , "Humidity.Max" , "Humidity.Min" , "Specific.Humidity" , "Temp.Mean.K" , "VPD.kPa" , "Precip.Mean.mm...day" , "NDVI" , "jrcStandingWater" , "irrWater" , "eddi14d" , "eddi180d" , "eddi1y" , "eddi270d" , "eddi2y" , "eddi30d" , "eddi5y" , "eddi90d" , "pdsi" , "spei14d" , "spei180d" , "spei1y" , "spei270d" , "spei2y" , "spei30d" , "spei5y" , "spei90d" , "spi14d" , "spi180d" , "spi1y" , "spi270d" , "spi2y" , "spi30d" , "spi5y" , "spi90d" , "z"))]

final_mirpir <- MIRPIR_meanTemp[c(c("MIRAll", "fold", "chirpsMean" , "Humidity.Max" , "Humidity.Min" , "Specific.Humidity" , "Temp.Mean.K" , "VPD.kPa" , "Precip.Mean.mm...day" , "NDVI" , "jrcStandingWater" , "irrWater" , "eddi14d" , "eddi180d" , "eddi1y" , "eddi270d" , "eddi2y" , "eddi30d" , "eddi5y" , "eddi90d" , "pdsi" , "spei14d" , "spei180d" , "spei1y" , "spei270d" , "spei2y" , "spei30d" , "spei5y" , "spei90d" , "spi14d" , "spi180d" , "spi1y" , "spi270d" , "spi2y" , "spi30d" , "spi5y" , "spi90d" , "z"))]
...
saveRDS(hypergrid_final, file = "Data/SDM/rf/hypergrid_final")
saveRDS(rf_performance, file = "Data/SDM/rf/rf_performance")



final_model  <-  ranger(
  formula  =  MIRAll  ~  .,
  data  =  final_mirpir[complete.cases(final_mirpir), -2],
  num.trees  =  2000 ,
  mtry  =  1,
  min.node.size  =  1,
  sample.fraction  =  0.6,
...
# Final Model Results
hypergrid <- readRDS("Data/SDM/rf/hypergrid_final_v2")
performance <- readRDS("Data/SDM/rf/rf_performance_v2")
final <-readRDS("Data/SDM/rf/final_model_v2")

pred0  <-  predict(final, data=mirpir_final[complete.cases(mirpir_final), -2]);  pred  <-  pred0$predictions[,1]
...
permutation_importance  <-  data.frame(variable  =  rownames(as.data.frame(final$variable.importance)),
                                       importance  = as.vector(final$variable.importance))
...
saveRDS(pd_df, file = "Data/SDM/rf/rd_pd")



pd_df <- readRDS("Data/SDM/rf/pd_df_v2")
output_directory <- "Data/SDM/rf/pdps"
pdp_plots <- list()
...
for (variable_name in unique(pd_df$variable)) {

  variable_data <- pd_df[pd_df$variable == variable_name, ]
